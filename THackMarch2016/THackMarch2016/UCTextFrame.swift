...
@objc(UCTextFrameAttributes)
class UCTextFrameAttributes: NSObject {
  // Basic info
  ...
  var textWidth: CGFloat {
    return UCTextFrame(attributes: self).width
  }
  
  var textHeight: CGFloat {
    return UCTextFrame(attributes: self).height
  }
}

@objc(UCTextFrame)
class UCTextFrame: NSObject {
  private let attributes: UCTextFrameAttributes
  private(set) var width: CGFloat = 0
  private(set) var height: CGFloat = 0
  
  init(attributes: UCTextFrameAttributes) {
